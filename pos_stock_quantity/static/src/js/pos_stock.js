...
odoo.define('pos_stock_quantity.pos_stock', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        after_load_server_data: function(){
            this.load_qty_after_load_product();
            return _super_pos.after_load_server_data.call(this);
        },
        load_qty_after_load_product: function () {
            let self = this;
            var ids = Object.keys(this.db.product_by_id).map(function (item) {
        ...
        compute_qty_in_pos_location: function (res) {
            ...
            res.forEach(function (item) {
                ...
                self.db.set_stock(product_id, item.quantity);
            })
        },
        recompute_qty_in_pos_location: function (product_ids, res) {
            ...
            out_of_stock_ids.forEach(function (id) {
                self.db.reset_product_stock(id);
            });

            res_product_ids.forEach(function (product_id) {
                self.db.delete_stock(product_id);
            });
    ...
    models.Orderline = models.Orderline.extend({
        ...
        check_reminder: function () {
            var self = this;
            var qty_available = this.pos.db.get_stock(this.product.id);
    ...
    screens.ProductListWidget.include({
        render_product: function (product) {
            if (this.pos.config.show_qty_available && product.type !== 'product') {
                this.pos.db.delete_stock(product.id);
            }
    ...
    screens.PaymentScreenWidget.include({
        ...
        sub_qty: function () {
            ...
            ids.forEach(function (id) {
                if (self.pos.db.get_stock(id)) {
                    self.pos.db.set_stock(id, -sub_qty_by_product_id[id]);
                }
            });
        }
    });
